...
public class ActivityController {
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name = "id", value = "出游活动id", required = true, dataType = "Long", paramType = "path"),
            @ApiImplicitParam(name = "activity", value = "出游活动详情实体类", required = true, dataType = "Activity", paramType = "body")
    })
    @ApiResponses({
            @ApiResponse(code=200,message="OK"),
    ...
    public @ResponseBody
    ResponseEntity<String> update(@PathVariable Long id, @RequestBody Activity activity) {
        return new ResponseEntity<String>("ok", HttpStatus.OK);
    }
    ...
    @GetMapping(value = "/{id}")
    public @ResponseBody
    ResponseEntity<Activity> get(@PathVariable Long id) {
